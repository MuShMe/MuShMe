...
def albumHook(albumname, imagefilename, artdata, albumartist, publisher, year):
  ...
  if flag == 0:
    ...
    g.database.execute("INSERT INTO albums(Album_pic, Album_name, Publisher, Album_year) VALUES('%s','%s','%s',%s)" % 
                      (imagefilename,albumname, publisher, year))
    g.conn.commit()
  
  g.database.execute("SELECT Album_id FROM albums WHERE Album_name='%s' AND Album_pic='%s'" % (albumname, imagefilename))
...
def artistHook(artistnames, albumname,albumid, date):
  ...
  for artistname in artistnames:
    ...
    if artistexists == False:
      ...
      g.database.execute("INSERT INTO album_artists VALUES (%s, %s)" % (artistid, albumid))
    else:
      g.database.execute("SELECT Artist_id FROM artists WHERE Artist_name='%s' AND Last_updated='%s'" % (artistname, date))
      artistid= g.database.fetchone()[0]
